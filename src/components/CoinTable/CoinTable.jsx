...
function CoinTable() {
    ...
    return (
        ...
        <div className='my-5 flex flex-col items-center justify-center gap-5 w-[80vh] mx-auto'>
            ...
            {/* table rows */}
            <div className='flex flex-col w-[80vh] mx-auto'>
                {isLoading && <div className='text-center text-white'>Loading...</div>}
                {data && data.map((coin) => (
                    <div
                        key={coin.id}
                        className='w-full bg-transparent text-white flex py-4 px-2 font-semibold items-center justify-between border-b border-gray-700'
                    >
                        {/* Coin Info */}
                        ...
                        {/* Price */}
                        <div className='basis-[25%] text-start'>
                            {coin.current_price.toLocaleString()}
                        </div>

                        {/* 24h Change */}
                        <div
                            className={`basis-[20%] text-start ${coin.price_change_percentage_24h >= 0 ? 'text-green-400' : 'text-red-500'
                                }`}
                        >
